...
/*
 The Application Timer library provides a user friendly way of using the 
 * Real Time Counter 1 (RTC1) peripheral to create multiple timer instances. 
 * The RTC uses the Low Frequency Clock (LFCLK). Most applications keep the 
 * LFCLK active at all times. When using a SoftDevice the LFCLK is 
 * always active. Therefor there is normally very little extra power 
 * consumption associated with using the application timer. As the clock 
 * is 32.768 kHz and the RTC is 24 bit, the time/tick resolution is limited, 
 * but it takes a substantial amount of time before the counter wrap around 
 * (from 0xFFFFFF to 0). The frequency of the RTC can be lowered by using the 
 * 12 bit (1/x) prescaler.
 * 
 * RTC rolls over every 512 seconds
 *
 * the softdevice uses RTC0
 * 
 * It is correct that the softdevice itself only requires RTC0, but as you've seen, 
 * some of the SDK modules use RTC1, most notably app_timer, 
 * which is again used by the connection parameters module.
 * 
...
#define COMP_CONFIG_REF 4   // TODO: Определиться
#endif
...
#define GPIOTE_CONFIG_NUM_OF_LOW_POWER_EVENTS 1 //used to be 4?
#endif
...
#define I2S_CONFIG_SCK_PIN 15//31
#endif
...
#define I2S_CONFIG_LRCK_PIN 18//30
#endif
...
#define NRF_LOG_DEFERRED_BUFSIZE 4096
#endif
...
#define NRF_LOG_BACKEND_MAX_STRING_LENGTH 1024
#endif
